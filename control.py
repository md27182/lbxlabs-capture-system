...
class ControlUI(QMainWindow):
    ...
    RED_STOP_COLOR = "#e8912e"
    GREEN_OK_COLOR = "#4caf50"
    ...
    """
    standard_label_font = "font-size: 9pt;"
    ...
    def __init__(self):
        ...
        self.setGeometry(400, 100, 1600, 900)
        self.set_dark_theme()
    ...
    def init_ui(self):
        ## Define UI sections
        machine_controls = QWidget()
        machine_settings = QWidget()
        camera_settings = QWidget()
        capture_sequence = QWidget()
        live_view = QWidget()
        last_image = QWidget()
        ...
        """)
        left_tabs.addTab(machine_controls, "Machine Controls")

        center_tabs = QTabWidget()
        center_tabs.setStyleSheet(left_tabs.styleSheet())
        center_tabs.setTabPosition(QTabWidget.North)
        center_tabs.addTab(last_image, "Latest Captured Image")
        center_tabs.addTab(live_view, "Live View")
        ...
        right_tabs.addTab(capture_sequence, "Capture Sequence")
        right_tabs.addTab(machine_settings, "Machine Settings")
        right_tabs.addTab(camera_settings, "Camera Settings")
...
#region
        ...
        machine_controls_layout = QVBoxLayout(machine_controls)

        # geo_label = QLabel("Current Geometry")
        # geo_label.setStyleSheet("font-size: 12pt;")
        # machine_controls_layout.addWidget(geo_label)

        geo_grid = QWidget()
        geo_grid_layout = QGridLayout(geo_grid)

        # theta_label = QLabel("θ ")
        # theta_label.setStyleSheet("font-size: 12pt;")

        self.geo = [{}, {}, {}, {}]
        for axis, axis_name in enumerate(["θ", "φ", "h", "f"]):
            axis_label = QLabel(axis_name + " ")
            axis_main_font_style = "font-size: 12pt;"
        ...
        keyboard_controls_layout.setContentsMargins(10, 5, 10, 5)  # Reduce bottom margin
        ...
        self.keyboard_controls_toggle.setStyleSheet("""
            QCheckBox::indicator {
                width: 18px;
                height: 18px;
                background-color: #2b2b2b;
        ...
        homing_layout.setContentsMargins(10, 5, 10, 5)  # Reduce top margin
...
#region
        ...
        ### Populate "Machine Settings" tab ###
        machine_settings_layout = QVBoxLayout()
        machine_settings_layout.addStretch()
        machine_settings.setLayout(machine_settings_layout)

#endregion
#region
#region

        ### Populate "Camera Settings" tab ###
        camera_settings_layout = QVBoxLayout()
        camera_settings_layout.addStretch()
        camera_settings.setLayout(camera_settings_layout)
...
#region
        ...
        capture_sequence_layout = QVBoxLayout()
        capture_sequence_layout.addStretch()
        capture_sequence.setLayout(capture_sequence_layout)
            ...
            while self.serial.in_waiting:
                ...
                if line:
                    ...
                    #     self.wrong_direction_flag = True
                        ...
                        print("Received invalid serial code from microcontroller: " + line)
                
            self.send_command('P')

            if estop_signal_detected and not self.estop_pressed:
                self.estop_pressed = True
                self.output_to_terminal("Emergency stop button has been pressed")
