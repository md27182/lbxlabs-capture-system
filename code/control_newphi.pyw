...
#region
        ...
        self.capture_button.clicked.connect(lambda: self.capture_image(format="IIQ", default_dest=True))
...
#region
        ...
        spin_set_layout.addWidget(folder_selector_widget)
        folder_selector_layout = QHBoxLayout(folder_selector_widget)
        folder_selector_layout.setContentsMargins(10, 5, 10, 5)

        folder_label = QLabel("Capture folder")
        folder_label.setStyleSheet(self.standard_label_font)
        folder_selector_layout.addWidget(folder_label)

        self.folder_path_line_edit = QLineEdit()
        self.folder_path_line_edit.setText(self.default_capture_directory)
        self.folder_path_line_edit.setReadOnly(True)
        self.folder_path_line_edit.setStyleSheet("""
            QLineEdit {
                background-color: #3a3a3a;
                font-size: 9pt;
            }
        """)
        folder_selector_layout.addWidget(self.folder_path_line_edit)

        def browse_capture_folder():
            folder = QFileDialog.getExistingDirectory(
                self,
                "Select Capture Folder",
                self.default_capture_directory
            )
            if folder:
                self.folder_path_line_edit.setText(folder)
                self.capture_directory = folder

        browse_folder_button = QPushButton("Browse")
        browse_folder_button.setStyleSheet(self.standard_button_style)
        browse_folder_button.clicked.connect(browse_capture_folder)
        folder_selector_layout.addWidget(browse_folder_button)
    ...
    def start_sequence(self):
        if self.camera is not None:
            ...
            self.output_to_terminal("Camera not connected, cannot start capture sequence")
    ...
    def move_capture_wait(self, theta, phi, h):
        ...
        moved = self.move_to_position([theta, phi, h])
